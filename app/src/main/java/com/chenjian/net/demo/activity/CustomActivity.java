...
public class CustomActivity extends Activity {
    ...
    private void customBean() {
        UrlParse urlParse = new UrlParse(UrlConst.BASE_URL).appendRegion(UrlConst.INFO_LIST);
        UrlParse paramParse = new UrlParse().putValue("page", "1").putValue("page_size", "3");
        NetHelper.post(urlParse.toStringOnlyHeader(), paramParse.toStringOnlyParam(), new NetSingleBeanListener<NetInfoListBean>() {
            @Override
            protected void onError(CallbackCode errorCode, NetRetBean netRetBean) {
                LogUtil.d(netRetBean.toString());
                requestError(netRetBean);
    ...
    private void custom() {
        UrlParse urlParse = new UrlParse(UrlConst.BASE_URL).appendRegion(UrlConst.INFO_LIST);
        UrlParse paramParse = new UrlParse().putValue("page", "1").putValue("page_size", "3");
        NetHelper.post(urlParse.toStringOnlyHeader(), paramParse.toStringOnlyParam(), new NetCustomBeanListener<NetPageBean, NetInfoBean>() {
            @Override
            protected void onError(CallbackCode errorCode, NetRetBean netRetBean) {
                LogUtil.d(netRetBean.toString());
                requestError(netRetBean);
    ...
    private void syncCustom() {
        new Thread() {
            ...
            public void run() {
                UrlParse urlParse = new UrlParse(UrlConst.BASE_URL).appendRegion(UrlConst.INFO_LIST);
                UrlParse paramParse = new UrlParse().putValue("page", "1").putValue("page_size", "3");
                NetRetBean netRetBean = NetHelper.postSync(urlParse.toStringOnlyHeader(),
                        paramParse.toStringOnlyParam(),
                        new SyncNetCustomBeanListener<NetPageBean, NetInfoBean>() {
                        });
